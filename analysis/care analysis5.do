/**************************************************************************************
*
*	PROGRAM TO ANALYSE LIFETIME PROFILES FOR SOCIAL CARE
*
...
clear all
global basedir =      "C:\justin\dev\SimPaths\output\sc analysis5\csv"
global zerocostsdir = "C:\justin\dev\SimPaths\output\sc analysis2\csv"
global naivedir =     "C:\justin\dev\SimPaths\output\sc analysis5b\csv"
...
merge m:1 idbenefitunit time using base0sc
drop _merge
...
rename id_benefitunit idbenefitunit
gsort idbenefitunit time
save "$outdir/naive0sc", replace
import delimited using "$naivedir/Person.csv", clear
rename *, l
rename id_person idperson
rename socialcareprovision socialcareprovision_p
gsort idbenefitunit time idperson
merge m:1 idbenefitunit time using naive0sc
drop _merge
gsort time idbenefitunit idperson
gen refbenefitunit = 0
replace refbenefitunit = 1 if (idbenefitunit != idbenefitunit[_n-1])
...
forvalues kk = 1/3 {
	...
	qui {
		...
		if (`kk'==2) {
			use "$outdir/naive1sc", clear
		}
		if (`kk'==3) {
			use "$outdir/base1sc", clear
		}
		...
		if (`kk'==1) {
			save "$outdir/zero2sc", replace
		}
		if (`kk'==2) {
			save "$outdir/naive2sc", replace
		}
		if (`kk'==3) {
			save "$outdir/base2sc", replace
		}
	}
}

use "$outdir/zero2sc", clear
save "$outdir/temp1", replace

use "$outdir/naive2sc", clear
merge 1:1 idperson time idspouse using temp1
keep if (_merge==3)
keep idperson time idspouse
save "$outdir/temp2", replace

use "$outdir/base2sc", clear
...
// save sample
keep idperson time
save "$outdir/selectedsc", replace
...
forvalues kk = 1/3 {
	...
	qui {
		...
		merge 1:1 idperson time using "$outdir/selectedsc"
		keep if (_merge==3)
		drop _merge
